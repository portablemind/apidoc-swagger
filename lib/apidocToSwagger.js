...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
  ...
  fileFields.forEach(function(field, index) {
    ...
    fileFieldIndexes.push(index);
  });

  // remove file fields
  verbs.parameter.fields.body = _.filter(verbs.parameter.fields.body, function(
    item
  ) {
    return item.type != 'file' && item.type != 'File';
  });
  ...
  if (verbs.parameter.fields.body && verbs.parameter.fields.body.length > 0) {
    params.push({
      in: 'body',
      name: 'body',
